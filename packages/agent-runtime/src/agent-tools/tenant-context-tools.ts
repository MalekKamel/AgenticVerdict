...
type TenantConfig = TenantContext["config"];
...
export interface TenantContextToolDeps {
  /** Optional cache for config slices (per-tenant). */
  configCache?: TenantScopedTtlCache<unknown>;
}

export function createTenantContextTools(deps: TenantContextToolDeps = {}): ITool[] {
  const cache =
  ...
  return [
    defineTool({
      ...
      description: "Return non-secret tenant profile fields from the active tenant context.",
      execute: async (_args, ctx) => {
        try {
          ...
          throw new AgentToolError(
            ...
            "Tenant context is required for get_tenant_profile",
            {
              cause: err,
            },
          );
    ...
    defineTool({
      ...
      description:
        "Return marketing KPI definitions and optional business positioning fields from TenantConfig.",
      execute: async (_args, ctx) => {
        try {
          const tenant = requireTenantContext();
          if (tenant.tenantId !== ctx.tenantId) {
    ...
    defineTool({
      name: "get_config",
      description:
        "Return a whitelisted TenantConfig slice (ai, features, localization, or marketing channels summary).",
