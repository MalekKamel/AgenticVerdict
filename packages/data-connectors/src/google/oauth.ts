 ...
 */
export async function validateGoogleAccessTokenForConnector(
  ...
  if (!res.ok) {
    ...
    throw new PlatformAuthError(connector, msg, { cause: body });
  }
  if (typeof body !== "object" || body === null) {
    throw new PlatformAuthError(connector, "tokeninfo returned a non-object body", { cause: body });
  }
 ...
 */
export async function refreshGoogleAccessTokenForConnector(
  ...
  if (!res.ok) {
    ...
    throw new PlatformAuthError(input.connector, msg, { cause: json });
  }
  if (typeof json !== "object" || json === null) {
    throw new PlatformAuthError(input.connector, "token endpoint returned a non-object body", {
      cause: json,
    });
  }
  const token = (json as { access_token?: unknown }).access_token;
  if (typeof token !== "string" || token.length === 0) {
    throw new PlatformAuthError(input.connector, "token response missing access_token", {
      cause: json,
    });
  }
