...
/** `ConnectorCredentials` keys consumed by {@link TikTokConnectorAdapter}. */
...
function parseSandboxFlag(credentials: ConnectorCredentials): boolean {
  const raw = credentials[tiktokCredentialKeys.sandbox];
  return typeof raw === "string" && raw.trim().toLowerCase() === "true";
}

export interface TikTokConnectorAdapterOptions extends BaseConnectorAdapterOptions {
  fetchImpl?: typeof fetch;
  /**
   * Per-HTTP-call limiter. Defaults to {@link createConnectorTokenBucket}("tiktok").
...
export class TikTokConnectorAdapter extends BaseConnectorAdapter {
  readonly connector: ConnectorType = "tiktok";
  ...
  constructor(options: TikTokConnectorAdapterOptions) {
    ...
    this.perRequestBucket =
      requestTokenBucket !== undefined ? requestTokenBucket : createConnectorTokenBucket("tiktok");
  }
