...
/** `ConnectorCredentials` keys consumed by {@link MetaConnectorAdapter}. */
...
export interface MetaConnectorAdapterOptions extends BaseConnectorAdapterOptions {
  /** Injected fetch (tests); defaults to global `fetch`. */
  fetchImpl?: typeof fetch;
  /**
   * Per Graph HTTP request limiter. Defaults to {@link createConnectorTokenBucket}("meta") (~200/hour).
 ...
 */
export class MetaConnectorAdapter extends BaseConnectorAdapter {
  readonly connector: ConnectorType = "meta";
  ...
  constructor(options: MetaConnectorAdapterOptions) {
    const { fetchImpl, requestTokenBucket, ...baseRest } = options;
    super("meta", { ...baseRest, tokenBucket: null });
    this.fetchImpl = fetchImpl ?? globalThis.fetch.bind(globalThis);
    this.perRequestBucket =
      requestTokenBucket !== undefined ? requestTokenBucket : createConnectorTokenBucket("meta");
  }
