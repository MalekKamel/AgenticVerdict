...
export abstract class BasePlatformAdapter implements PlatformAdapter {
  ...
  protected constructor(platform: PlatformType, options: BasePlatformAdapterOptions) {
  ...
  protected abstract doAuthenticate(credentials: PlatformCredentials): Promise<void>;
  ...
  async fetchMetrics(dateRange: DateRangeIso): Promise<unknown> {
    ...
    const cacheKey =
      this.cache !== null
        ? buildAdapterCacheKey({ tenantId: this.tenantId, platform: this.platform, dateRange })
        : null;
    ...
    try {
      ...
      if (this.cache !== null && cacheKey !== null) {
        try {
          const ttl = this.cacheTtlSeconds ?? defaultAdapterCacheTtlSeconds(this.platform);
