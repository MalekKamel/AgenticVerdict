...
export interface PlatformHealthReport {
  ...
  readonly adapter: PlatformAdapterMetricSnapshotJson;
  readonly status: "ok" | "degraded" | "unknown";
}

interface PlatformAdapterMetricSnapshotJson {
  successCount: number;
  failureCount: number;
  cacheHits: number;
  cacheMisses: number;
...
export async function collectInfrastructureHealth(options: InfrastructureHealthOptions): Promise<{
  ...
  const platformReports: PlatformHealthReport[] = platforms.map((platform) => {
    const snap = options.metrics.snapshotForPlatform(platform);
    const score = healthScoreFromMetrics(snap);
    const adapterJson: PlatformAdapterMetricSnapshotJson = {
    ...
    const status =
      snap.successCount + snap.failureCount === 0
        ...
        : score >= 80
          ? ("ok" as const)
          : ("degraded" as const);
    return { platform, healthScore: score, adapter: adapterJson, status };
  });
  ...
  const overallDegraded =
    ...
    platformReports.some((p) => p.status === "degraded");
