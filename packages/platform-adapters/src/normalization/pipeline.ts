...
import { parseNormalizedPlatformSnapshot } from "./schema";
import type {
...
export interface NormalizationPipelineResult {
  readonly snapshot: NormalizedPlatformSnapshot;
...
export function runNormalizationPipeline(
  ...
  const working: NormalizedPlatformSnapshot = {
    platform: snapshot.platform,
    dateRange: { ...snapshot.dateRange },
    records,
  ...
  const parsed = parseNormalizedPlatformSnapshot(working);
