...
function placeholderPatternCounts(value: string): Map<string, number> {
  ...
  // Extract ICU variable names from both simple {var} and complex {var, plural/select, ...} forms.
  // For {count, plural, one {# item} other {# items}} → extracts "count"
  // For {name} → extracts "name"
