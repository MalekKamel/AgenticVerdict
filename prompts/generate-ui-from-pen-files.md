...
The design system foundation has been established using .pen files created with Pencil (pendil.dev). These design artifacts are located in the `design/` directory and follow the structure documented in `/design/README.md`.
...
- Document the structure and contents of existing .pen files in `design/`
...
- `get_guidelines()` — Loading **Code** and **Tailwind** (and other) guides before implementing application UI
- `get_screenshot()` — Visual verification of components

Document explicitly that **Pencil MCP does not emit application source files**; the workflow is MCP inspection + guidelines + workspace implementation (see `/design/docs/generation/ui-generation-cheatsheet.md#mcp-first-design-to-code-workflow-repo-ssot`).
...
- **MCP-first UI implementation**: Before writing or changing React/UI code for a `.pen`-backed feature, use MCP to inspect the design and load `get_guidelines` (**Code**; **Tailwind** when using Tailwind). Do not invent design tokens from memory; map MCP output to CSS variables / theme per `/design/docs/generation/ui-generation-cheatsheet.md#mcp-first-design-to-code-workflow-repo-ssot`.
