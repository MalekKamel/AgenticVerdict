...
**MCP-first workflow (repo)**: Examples assume design values were taken from Pencil MCP (`batch_get`, `get_variables`) and that implementers loaded **`get_guidelines({ name: "Code" })`** (and **`Tailwind`** when applicable) before writing UI. Pencil MCP does not write `.tsx` files; see [cheat sheet](./ui-generation-cheatsheet.md#mcp-first-design-to-code-workflow-repo-ssot).
...
- No invented colors or spacing — trace tokens to MCP output, then map to CSS variables / theme
...
**File**: `designsystem/atoms.lib.pen`
...
**File**: `designsystem/design-tokens.lib.pen`
 ...
 * Badge Component
 *
 * Translated from designsystem/atoms.lib.pen
 *
 * Design decisions:
...
**File**: `designsystem/molecules.lib.pen`
