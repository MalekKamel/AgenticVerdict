...
1. **Pencil MCP does not emit application source files** — it accesses `.pen` designs. React/TypeScript is implemented in the repo using MCP-extracted specs and Pencil **`get_guidelines`** (see [cheat sheet](./ui-generation-cheatsheet.md#mcp-first-design-to-code-workflow-repo-ssot)).

2. **Before Phase 1 (tokens)**, load **`get_guidelines({ category: "guide", name: "Code" })`**; when styling with Tailwind, load **`get_guidelines({ category: "guide", name: "Tailwind" })`**.

3. **All token values** must trace to **`get_variables` and/or `batch_get`** output — never guessed from memory.
...
See the `design` directory for .pen file examples:
