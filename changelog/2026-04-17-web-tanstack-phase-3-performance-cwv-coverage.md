...
**Verification run:** `pnpm exec turbo run lint typecheck --filter=@agenticverdict/frontend`, `pnpm --filter @agenticverdict/frontend test`, `pnpm --filter @agenticverdict/frontend test:coverage`, `pnpm --filter @agenticverdict/frontend build`, `ANALYZE=true pnpm --filter @agenticverdict/frontend build` (local bundle treemap), `pnpm --filter @agenticverdict/frontend test:e2e` (recommended after a11y spec change).

---

## Summary

- **P3-1 Code splitting:** Auth routes (`/$locale/auth/*`) and **`/$locale/dashboard`** now use **`lazyRouteComponent`** with colocated **`-*.page.tsx`** modules so locale home and shell stay on the critical path while auth/dashboard chunks load on demand. **`rollup-plugin-visualizer`** runs when **`ANALYZE=true`** and writes **`dist/bundle-stats.html`** (gitignored via `dist/`). Script: **`pnpm --filter @agenticverdict/frontend run build:analyze`**.
...
- **CI:** **Quality** job runs **`pnpm --filter @agenticverdict/frontend exec vitest run --coverage`** and **`pnpm --filter @agenticverdict/frontend build`** so Phase 3 gates run on every PR.
