...
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Stack / routing SSOT | `apps/frontend/README.md` version table; `src/router.tsx` imports `./routeTree.gen`; root `.gitignore` ignores `routeTree.gen.ts`; CI runs `pnpm --filter @agenticverdict/frontend build` (`.github/workflows/ci.yml`)                                            | **Complete** for documented approach (generated tree, not committed).                                                        |
...
| White-label           | `apps/frontend/src/components/providers/TenantBrandedThemeProvider.tsx`, `apps/frontend/src/lib/tenant/tenant-branding.ts`          | **Partial** — **reference UUID** mapping; **TenantConfig-driven** theme **deferred**.                        |
...
4. **`apps/frontend/src/lib/tenant/tenant-branding.ts`** — UUID → packaged brand tokens; **TenantConfig**-driven fetch not implemented (risk of theme flash noted in changelog).
