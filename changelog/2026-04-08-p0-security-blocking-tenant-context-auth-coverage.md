...
- **Tenant context:** After JWT validation, the API loads `TenantConfig` per tenant, attaches `FastifyRequest.tenantContext`, and wraps route handlers with `runWithTenantContext` when that context exists so `getTenantContext()` works for the full handler (required because `AsyncLocalStorage.enterWith` after an async `preHandler` does not carry into Fastify’s handler invocation).
- **Security tests:** Added **25** matrix-style API tests (`it()` blocks in `auth-security-matrix.test.ts`) plus **2** focused tenant-context middleware tests; existing contract and route tests unchanged in behavior aside from needing tenant JSON for JWT tenants under Vitest.
...
- **`src/middleware/auth-security-matrix.test.ts`** — **25** scenarios (count = `grep -c "it("` on this file): unauthenticated / bad JWT sweeps over key GET routes, role-based 403 (reports write, translations write, admin workflows), optional JWT, JWT garbage, admin-only mini-app, missing tenant config.
- **`test-fixtures/tenant-configs/*.json`** — tenant configs for JWT tenants used in API tests (including Masafh and demo UUIDs aligned with `api.contract.test.ts`).
...
- **`vitest.config.ts`** — sets `TENANT_CONFIG_DIR` to `test-fixtures/tenant-configs` for isolated, hermetic config loading in API tests.
...
- **Config must exist for JWT tenants:** Valid JWT + unknown/missing tenant file → **403** `tenant_config_not_found` (fail closed for tenant resolution).
...
- **Operational:** Production deployments must keep `JWT_SECRET` / `JWT_SECRET_FILE` and `TENANT_CONFIG_DIR` consistent with issued tokens and tenant JSON files.
