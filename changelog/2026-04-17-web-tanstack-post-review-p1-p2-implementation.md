...
**Verification run:** `pnpm exec turbo run lint typecheck --filter=@agenticverdict/web --filter=@agenticverdict/api --filter=@agenticverdict/types`, `pnpm --filter @agenticverdict/web exec vitest run --coverage`, `pnpm --filter @agenticverdict/api exec vitest run`, `pnpm --filter @agenticverdict/web build`, `pnpm --filter @agenticverdict/web exec playwright test e2e/protected-routes.spec.ts`, optional `cd apps/frontend && npx @lhci/cli@0.14.0 autorun --config=./lighthouserc.cjs` (after build).
...
- **CompanyConfig-driven branding:** optional **`ui.brand`** on **`CompanyConfig`** ([`packages/config/src/schemas/company-ui.ts`](../packages/config/src/schemas/company-ui.ts)); Masafh fixture updated in [`configs/companies/11111111-1111-4111-8111-111111111111.json`](../configs/companies/11111111-1111-4111-8111-111111111111.json); public **`tenant.getBranding`** serves validated brand tokens from disk config; [`TenantBrandedThemeProvider`](../apps/frontend/src/components/providers/TenantBrandedThemeProvider.tsx) merges API brand with packaged fallbacks from [`tenant-branding.ts`](../apps/frontend/src/lib/tenant/tenant-branding.ts).
- **Subdomain / slug → tenant UUID:** **`tenant.resolveSlug`** resolves **`companies.slug`** (case-insensitive); web sets **`VITE_PUBLIC_TENANT_BASE_DOMAINS`** (e.g. `localhost` for `acme.localhost`), uses **`extractTenantSlugFromHost`** from **`@agenticverdict/core`**, and extends [`getEffectiveTenantId`](../apps/frontend/src/lib/tenant/tenant-resolution.ts) + [`TenantProvider`](../apps/frontend/src/providers/TenantProvider.tsx) so **`x-tenant-id`** aligns with slug-derived tenants before login.
