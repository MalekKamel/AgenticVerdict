...
| `/apps/web/src/lib/api/trpc-client.ts`                 | tRPC client configuration            |
| `/apps/web/src/lib/api/auth-api.ts`                    | Auth API wrapper with error handling |
| `/apps/web/src/components/providers/trpc-provider.tsx` | tRPC + React Query provider          |
| `/apps/web/src/hooks/useAuthMutation.ts`               | Auth mutation hooks                  |
| `/apps/web/src/hooks/useSessionQuery.ts`               | Session query hooks                  |
| `/apps/web/src/lib/api/AUTH_API_USAGE.md`              | Comprehensive usage guide            |
...
| `/apps/web/src/lib/api/auth-api.ts` | Fixed imports to use types package |
...
1. Export `AppRouter` type from `@agenticverdict/api`
2. Uncomment real implementation in `auth-api.ts`
...
1. **Wrap app with `TRPCProvider`** in root layout
2. **Test mock implementations** in development
3. **Review error messages** for i18n compatibility
4. **Add unit tests** for mutation/query hooks

### Short-term (Backend Ready)

1. **Implement tRPC router** in `/apps/api/src/routers/auth.router.ts`
2. **Export `AppRouter` type** from `@agenticverdict/api`
3. **Uncomment real implementations** in `auth-api.ts`
...
- **`/apps/web/src/lib/api/AUTH_API_USAGE.md`** - Developer guide with examples
