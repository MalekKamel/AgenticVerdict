/**
 * AuthError — `design/system/molecules.lib.pen` via `@agenticverdict/ui`.
 */
...
export const AuthError = forwardRef<HTMLDivElement, AuthErrorProps>(
  (
    ...
    const isWarningSurface = alertVariant === "warning";
    ...
    return (
      <div ref={errorRef} className={className} tabIndex={-1}>
        {isWarningSurface ? (
          ...
          >
            <p className="m-0 mb-1 font-semibold">{t("common.error")}</p>
            <Typography variant="body-sm" as="span" color="secondary">
              {errorMessage}
            </Typography>
            {actionRow}
          </div>
