...
export function TemplateBrowser({ onSelectTemplate, onStartFromScratch }: TemplateBrowserProps) {
  ...
  if (error) {
    return (
      <Stack gap="md">
        <Title order={3}>{t("chooseStartingPoint")}</Title>
  ...
  return (
    <Stack gap="md">
      ...
      {isLoading ? (
        ...
        <Text c="dimmed">{t("noTemplatesFoundAction")}</Text>
      ) : (
            ...
            <Card key={template.id} withBorder radius="md">
              <Stack gap="xs">
                ...
                <Text size="xs" c="dimmed">
                  {t("connectorCount", { count: template.connectorCount })}
                </Text>
