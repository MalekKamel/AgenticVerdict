...
export function TemplatePreviewModal({ templateId, onClose, onApply }: TemplatePreviewModalProps) {
  const t = useTranslations("insights.templates");
  const { data: template, isLoading } = useTemplateDetail(templateId || "");

  const isOpen = !!templateId;

  return (
    <Modal opened={isOpen} onClose={onClose} title={t("previewTitle")} size="lg">
      {isLoading ? (
        <Text c="dimmed">{t("loadingDetails")}</Text>
      ) : template ? (
        <Stack gap="md">
          <Group>
          ...
          <Stack gap="xs">
            <Title order={5}>{t("connectorsMetrics")}</Title>
          ...
          <Stack gap="xs">
            <Title order={5}>{t("delivery")}</Title>
            <Text size="sm">
              {t("formatLabel")}: {t(`format.${template.delivery.format}`)}
              {template.delivery.enableWebhook && ` · ${t("webhookEnabled")}`}
            </Text>
          </Stack>
          ...
          <Group justify="flex-end">
            ...
            <Button onClick={() => onApply(template.id)}>{t("applyAndCustomize")}</Button>
          </Group>
        </Stack>
      ) : (
        <Text c="dimmed">{t("templateNotFound")}</Text>
      )}
    </Modal>
  );
}
