...
### 1. Use the app shell providers

The root route wraps the tree with **`Providers`** (`src/components/Providers.tsx`), which already includes `@trpc/react-query` (`trpc.Provider` + shared `trpcClient`), React Query, and session/tenant providers. Add feature-specific providers next to that shell rather than a separate legacy tRPC wrapper.

```tsx
import { Providers } from "@/components/Providers";

// In __root.tsx (or equivalent), the app already uses <Providers>{children}</Providers>
```
...
1. **Keep the app shell** (`Providers` in `src/components/Providers.tsx`) as the single integration point for tRPC + React Query
...
Ensure the workspace dependency is installed (`@agenticverdict/api` in `apps/frontend/package.json`) and run `pnpm install` from the repo root. The shared `AppRouter` type is exported from `@agenticverdict/api/trpc`.
