...
flowchart LR
  ...
  subgraph AgenticVerdict
    ...
    PA[@agenticverdict/platform-adapters]
    C[(Cache\nMemory / Upstash)]
...
2. `fetchMetrics(dateRange)` enters `BasePlatformAdapter`:
   - optional token bucket,
   - cache get by key (`tenantId`, `platform`, `dateRange`),
   - circuit breaker,
   - backoff-wrapped vendor calls in `fetchRawMetrics`.
3. Raw payload returned to caller; `normalizeData` produces `NormalizedPlatformSnapshot`.
...
- **Multi-tenancy:** `tenantId` is required on `BasePlatformAdapterOptions` (non-empty string) so cache keys and observability never fall back to a shared default segment.
...
- `@agenticverdict/types` — `PlatformType` and shared enums.
