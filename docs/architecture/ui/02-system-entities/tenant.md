...
The **Tenant/Tenant** entity is the foundational organizational unit in the AgenticVerdict platform. It represents either a **direct business** (end consumer) or an **agency partner** (managing multiple client tenants). All data, configurations, users, and resources are scoped to a tenant, ensuring complete multi-tenant isolation.

**Key Concept:** A tenant is the single source of truth for tenant-specific behavior, branding, localization, and feature configuration. Every operation in the platform executes within a tenant context.
...
- **Business Owners:** Configure tenant settings, manage users, control branding
- **Agency Partners:** Switch between client tenants, oversee multiple tenants
- **Administrators:** Manage tenant lifecycle, billing, access control

### Business Functions

- Multi-tenant data isolation and security
- Tenant-specific localization (language, timezone, currency)
...
| ----------- | -------- | --------------------- | ------------------------------------ | -------------------------------------- |
| `tenantId`  | UUID     | Required, unique      | `abc-123-def`                        | Unique tenant identifier (primary key) |
| `name`      | String   | Required, min 2 chars | "Masafh"                             | Tenant display name                    |
...
| Property                  | Type   | Validation          | Display Format         | Description                    |
| ------------------------- | ------ | ------------------- | ---------------------- | ------------------------------ |
| `branding.logoUrl`        | String | URL, optional       | Logo image             | Tenant logo for white-labeling |
...
| `business.competitors`       | Array  | Optional   | Tag input                 | Competitor businesses     |
...
- **None** — Tenant/Tenant is the root entity
...
| ----------------- | ----------------- | --------------------------------------------- |
| **TenantConfig**  | Association       | Tenant loads configuration from file/database |
...
- **Input:** Tenant name, type, initial admin user
- **Validation:** Unique tenant ID, valid email
...
```json
{
  ...
  "tenant.switcher.label": "Switch Tenant",
  "tenant.switcher.currentTenant": "Current: {tenantName}"
}
```
...
| **Tenant Settings**    | `/settings/tenant`   | Configure tenant properties     | TenantSettingsForm              |
...
| **TenantSettingsForm** | Form for editing tenant properties             | `tenant`, `onUpdate`                              |
...
function AgencyDashboard() {
  ...
  return (
    <DashboardLayout>
      <TenantSwitcher
        current={currentTenant.tenantId}
        tenants={availableTenants}
        onSelect={(tenantId) => setCurrentTenant(tenantId)}
      />
      <TenantDashboard tenantId={currentTenant.tenantId} />
    </DashboardLayout>
  )
}
```
...
function InsightList() {
  ...
  // tenant.tenantId automatically included in request
