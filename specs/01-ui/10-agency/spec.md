...
As an agency partner offering white-label services to my clients, I need to customize the branding elements (logo, colors, tenant name) that appear on reports and client-facing views so that my end clients see my agency brand instead of AgenticVerdict.
...
1. **Given** I am logged in as an agency partner, **When** I navigate to Agency Settings > Branding, **Then** I should see options to upload a logo, select brand colors, and set a tenant name
...
- **FR-010**: System MUST support white-label branding customization including logo upload, color selection, and tenant name configuration
...
- **AgencyBranding**: Custom branding configuration (logo, colors, tenant name) applied to client-facing views and reports
...
- White-label branding configuration is stored in the TenantConfig schema for agency partner tenants
