...
**Workflow**: design (`*.pen`) → tokens (`design/system/design-tokens.lib.pen`) → `@agenticverdict/ui` → auth feature code in `apps/web`. See `design/docs/generation/ui-generation-quick-reference.md`.

- **Authoritative visuals**: Atoms and molecules are consolidated in `design/system/atoms.lib.pen` and `design/system/molecules.lib.pen`. **Do not** hand-roll one-off styles for auth; compose from the shared library.
...
- [ ] T015 [P] [US0] Create AuthLayout component in `apps/web/src/components/auth/AuthLayout.tsx` using `Card` + `Typography` from `@agenticverdict/ui` (structure per `design/system/molecules.lib.pen` + typography tokens)
...
- [ ] T029 [P] [US1] Create PasswordInput component in `apps/web/src/components/auth/PasswordInput.tsx` using `Input` + `FormFieldLabel` from `@agenticverdict/ui` (see `design/system/atoms.lib.pen`; visibility toggle, ARIA)
- [ ] T030 [P] [US1] Create AuthError component in `apps/web/src/components/auth/AuthError.tsx` using `Alert` from `@agenticverdict/ui` (`design/system/molecules.lib.pen`; ARIA alerts)
