...
**AC-1.7.0b**: Default data freshness (TTL) matches platform policy

- **Verification**: Unit or config test against `defaultAdapterCacheTtlSeconds` (meta/tiktok **120s**, ga4 **300s**, gsc/gbp **600s**)
- **Evidence**: Cached payloads expire and refetch within expected windows under load tests
- **Owner**: Backend Developer
