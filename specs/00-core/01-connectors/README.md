...
**Product requirement (cross-reference):** Every `BaseConnectorAdapter` (and mocks used in its place) MUST be constructed with a **non-empty `tenantId`** from request or job context — see [`requirements.md`](../../../docs/05-project-management/requirements.md) §Platform integration requirements and [`operations/SECURITY.md`](./operations/SECURITY.md) (multi-tenancy and verification matrix).
