...
8. **HTTP API & external contracts** (4 tasks) — specifications in [API_SPECIFICATIONS.md](./API_SPECIFICATIONS.md); implementation tracked under [Remediation Plan](/specs/00-core/REMEDIATION_PLAN.md) **R-1–R-6**
...
**Verdict structure:** Use the unified **`MarketingVerdict`** type from `@agenticverdict/types` (see [Remediation Plan](/specs/00-core/REMEDIATION_PLAN.md) **R-7**) — **do not** maintain a parallel `Verdict` interface. Phase 3 report templates bind directly to this schema (optional `reportMetadata` for layout hints).
